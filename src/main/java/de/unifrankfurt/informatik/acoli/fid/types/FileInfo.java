...
public class FileInfo implements Serializable {
	...
	public void setModelMatchingsUnknown() {
		...
		ModelMatch modelMatch = new ModelMatch(ModelType.UNKNOWN);
	...
	public TreeNode getAllModelMatchingsAsTree() {
   	 ...
   	 if (fileResults != null && !fileResults.isEmpty()) {
   		...
   		for (Integer col : columns) {
   		 ...
   		 FileResult xyz = new FileResult();
	    	 xyz.setFoundTagOrClass(ModelType.UNKNOWN.name());
	...
	public boolean isConllFile() {
		return (fileFormat == FileFormat.CONLL);
	}
	
	
	public boolean isRDFFile() {
		return (fileFormat == FileFormat.RDF);
	}
	
	
	public boolean isXMLFile() {
		return (fileFormat == FileFormat.XML);
	}
	...
	public void setSelectedModelForColumn(int col, ModelType selectedColModel) {
		...
		for (ModelMatch mm : getAllModelMatchingsForColumn(col)) {
			if(mm.getModelType() == selectedColModel) {
				Utils.debug("true");
				mm.setSelected(true);
	...
	/**
	 * Change the process state of a resource depending on the quality of its results. If the detected resource type is unknown then assign the process state
	 * DISABLED. If on the other hand model information has been detected but coverage values are very low then assign the process
	 * state CHECK. Both states CHECK and DISABLED will disqualify a resource for later RDF export (RDF export is done for ACCEPPTED state resources only)
	 * Also the CHECK state is a virtual state that is assigned automatically after parsing. It changes if the user edits or accepts the resource.
	 * The DISABLED state can be assigned to the resource by the user in the web-interface.
	 */
	public void checkOrDisableProcessState () {
		
		// Do not alter process states unprocessed, accepted, edited, accepted or disabled
		if (this.processState != ProcessState.PROCESSED) return;
		...
		if (this.getResourceType() == ResourceType.CORPUS) {
			...
			if (!minCoverageSucceeded) this.processState = ProcessState.CHECK;
		}
	}
	...
	public ParseResult getParseResult() {
		...
		if ((this.getModelMatchings().isEmpty() ||
			(this.getModelMatchings().size() == 1) && (this.getModelMatchings().get(0).getModelType() == ModelType.UNKNOWN)) &&
	 ...
	 * 
	 */
	public void updateLanguageMatchings(ArrayList <LanguageMatch> oldLanguages, ArrayList <LanguageMatch> newLanguages, FileFormat fileFormat) {
	...
	public ResourceType getResourceType(){
		...
		if (fileFormat == FileFormat.CONLL) return ResourceType.CORPUS;
		...
		for (ModelMatch mm : getSelectedModels()) {
			if (mm.getModelType() != ModelType.UNKNOWN) return ResourceType.CORPUS;
		}
