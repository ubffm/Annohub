...
public class UrlBroker {
	...
	/**
	 * Add linghub attributes to ResourceInfo objects by quering Linghub RDF local file
	 * (http://linghub.org/sparql SPARQL endpoint)
	 * @param SPARQL query string
	 * @param useLocalFile selects local Linghub RDF or http://linghub.org/sparql for retrieving info about Linghub resources 
	 * @param resourceMap
	 */
	public void sparqlLinghubAttributes(boolean alwaysLocal, List<ResourceInfo> resourceList) {
		...
		for (String q : queries) {
			...
			for (ResourceInfo resourceInfo : resourceList) {
				...
				if  (  resourceInfo.getDataURL() == null
					...
					|| resourceInfo.getLinghubAttributes().getMetadataSource() != MetadataSource.NONE
					)  continue;
				...
				for (TupleQueryResult rs : resultSets) {
					...
					try {
						while (rs.hasNext()) {
							...
							if (title != null) {
								resourceInfo.getLinghubAttributes().setTitle(title.stringValue());
							}
							Value description = (Value) x.getValue("description");
							if (description != null) {
								success = true;
								resourceInfo.getLinghubAttributes().setDescription(description.stringValue());
							}
							Value language = (Value) x.getValue("language");
							if (language != null) {
								success = true;
								resourceInfo.getLinghubAttributes().setDcLanguageString(language.stringValue());
							}
							Value rights = (Value) x.getValue("rights");
							if (rights != null) {
								success = true;
								resourceInfo.getLinghubAttributes().setRights(rights.stringValue());
							}
							Value date = (Value) x.getValue("date");
							if (date != null) {
								success = true;
								resourceInfo.getLinghubAttributes().setDate(new Date(date.stringValue()));
							}
							Value creator = (Value) x.getValue("creator");
							if (creator != null) {
								success = true;
								resourceInfo.getLinghubAttributes().setCreator(creator.stringValue());
							}
							
							Value contributor = (Value) x.getValue("contributor");
							if (contributor != null) {
								success = true;
								resourceInfo.getLinghubAttributes().setContributor(contributor.stringValue());
							}
							
							Value subject = (Value) x.getValue("subject");
							if (subject != null) {
								success = true;
								resourceInfo.getLinghubAttributes().addKeyword(subject.stringValue());
							}
							
							Value homepage = (Value) x.getValue("homepage");
							if (homepage != null) {
								success = true;
								resourceInfo.getLinghubAttributes().setWebpage(homepage.stringValue());
							}
						
							
							if (success) {
								resourceInfo.getLinghubAttributes().setMetadataSource(MetadataSource.LINGHUB);
							}
	 ...
	 * @param resourceMap
	 */
	private static void readUrlseedFile(File urlseedFile, HashMap <String, ResourceInfo> resourceMap) {
		
		// Omit seed file if missing !
		if (urlseedFile == null || !urlseedFile.exists()) return;
