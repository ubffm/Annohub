...
public class GenericXMLFileHandler implements XMLFileHandlerI{
	...
	private int makeConllSentenceCount = 15000;
	...
	public void parse(ResourceInfo resourceInfo, Worker fidWorker) throws XMLStreamException, IOException {
		...
		if (!resourceInfo.getFileInfo().isXMLFile()) {
			finishWithErrors(resourceInfo, "XMLParser error : wrong fileFormat '"+ resourceInfo.getFileInfo().getFileFormat()+"'");
			Utils.debug("XMLFileHandler : wrong fileFormat '"+ resourceInfo.getFileInfo().getFileFormat()+"'");
			return;
		}
		...
		try 
		{
		  makeConllSentenceCount = fidWorker.getConfiguration().getInt("Processing.GenericXmlFileHandler.makeConllSentenceCount");
		} catch (Exception e){}
		try 
		{
...
//			success =false;
		...
		*/
		...
		// update file format
		resourceInfo.getFileInfo().setFileFormat(FileFormat.CONLL);
