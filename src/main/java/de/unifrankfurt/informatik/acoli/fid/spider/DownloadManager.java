...
public class DownloadManager {
	 ...
	 */
	public void getResource(int workerID, ResourceInfo resourceInfo, boolean emptyFolder) {
		...
		switch (protocol) {
			...
			*/
			...
			default :
				...
				// Register resource in the resource database if download was successful
				resourceManager.registerResource(resourceInfo, response);	
		}
	...
	// Http exception handling
		...
		if (e instanceof SocketTimeoutException || e instanceof  java.net.SocketTimeoutException
		 || e instanceof ConnectTimeoutException) {
					System.out.println(resourceInfo.getDataURL()+" timeout after "+httpTimeout+" sec !");
					resourceInfo.getFileInfo().setErrorMsg(StringUtils.substring(e.getMessage(),0,100));
			}
			else if (e instanceof UnknownHostException) {
					System.out.println(resourceInfo.getDataURL()+" is unknown !");
					resourceInfo.getFileInfo().setErrorMsg(StringUtils.substring(e.getMessage(),0,100));
			}
			else if (e instanceof HttpHostConnectException) {
					System.out.println(resourceInfo.getDataURL()+" could not establish connection !");
					resourceInfo.getFileInfo().setErrorMsg(StringUtils.substring(e.getMessage(),0,100));
			} else {
					e.printStackTrace();
					resourceInfo.getFileInfo().setErrorMsg(StringUtils.substring(e.getMessage(),0,100));
	...
	public File simpleFileDownload(String url, File targetDirectory) {
		...
		try {
			...
			if (fileEntity == null) return null;
			 ...
			 //System.out.println("Downloading file to "+downloadDirectory.getAbsolutePath());
