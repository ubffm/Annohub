...
public class OptimaizeLanguageTools1 {
	...
	private static ArrayList<String> languageProfileLanguages = new ArrayList<String>();
	...
	public static void createOptimaizeLanguageProfile(File trainingData, String languageIso639_3, File targetDirectory) {
		...
		TextObject inputText = textObjectFactory.create()
		        .append(FileUtils.fileRead(trainingData));
	...
	public static void initLanguageDetector() {
		...
		try {
			...
			System.out.println("Loading builtin Optimaize language profiles : ("+languageProfiles.size()+")");
			...
			for (LanguageProfile x : languageProfiles) {
				
				// convert isocode to 639-3
				String iso6393 = TikaTools.getISO639_3CodeFromISOCode(x.getLocale().getLanguage());
				if (iso6393 != null) languageProfileLanguages.add(iso6393);
			}
	...
	public static ArrayList <LanguageMatch> detectAllISO639_3Languages(String text) {
	    ...
	    return detectAllISO639_3Languages(splitSentences);
	}
	...
	public static ArrayList<String> getLanguageProfileLanguages() {
		return languageProfileLanguages;
	}
