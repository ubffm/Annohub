...
public class OntologyManager {
	...
	public void updateOliaModels () {
		...
		for (ModelType modelType : modelDefinition.getModelDefinitions().keySet()) {
			...
			for (ModelInfo model : modelDefinition.getModelDefinitions().get(modelType).getModelFiles()) {
				...
				downloadManager.getResource(workerId, resourceInfo, false);
		...
		if (newOrChangedModels.contains(ModelType.BLL)) {
		...
		rfl.addAll(executer.getResourceManager().getAllResourcesRI());
	...
	private void loadOliaModel(ModelInfo model) {
		...
		try {
			...
			if (!(model.getModelType() == ModelType.BLL)) {
				modelParser.reset();
