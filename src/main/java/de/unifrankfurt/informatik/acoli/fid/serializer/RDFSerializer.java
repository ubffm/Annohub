...
public class RDFSerializer {
	...
	private static Model serializeResourceInfo2RDFModel (
		...
		HashMap<FileFormat,String> fileFormatMap = new HashMap<FileFormat,String>();
		fileFormatMap.put(FileFormat.CONLL, "application/rdf+xml");
		fileFormatMap.put(FileFormat.RDF, "application/rdf+xml");
		fileFormatMap.put(FileFormat.XML, "text/xml");
		...
		for (ResourceInfo rs : resourceInfoList) {
			...
			if (!doneDatasets.keySet().contains(datasetURL)) {
				...
				if (rs.getLinghubAttributes().getMetadataSource() != MetadataSource.NONE) {
					
					// add comment for metadate source
					mainDataset.addProperty(comment,"Metadata generated from "+rs.getLinghubAttributes().getMetadataSource());
					
					if (rs.getLinghubAttributes().getMetadataSource() == MetadataSource.LINGHUB) {
				...
				if (!Utils.filterNa(rs.getLinghubAttributes().getTitle()).isEmpty()) {
					mainDataset.addProperty(dcTitle, rs.getLinghubAttributes().getTitle());
				}
				
				// UB title
				if (!publicVersion) {
				if (!rs.getLinghubAttributes().getUbTitle().isEmpty()) {
					mainDataset.addProperty(ubTitle, rs.getLinghubAttributes().getUbTitle());
				} else {
					// copy title to ubTitle
					if (!Utils.filterNa(rs.getLinghubAttributes().getTitle()).isEmpty()) {
						mainDataset.addProperty(ubTitle, rs.getLinghubAttributes().getTitle());
					}
				}
				}
				
				if (!Utils.filterNa(rs.getLinghubAttributes().getRights()).isEmpty())
					mainDataset.addProperty(dcRights, rs.getLinghubAttributes().getRights());
				...
				for (String subject : rs.getLinghubAttributes().getKeywords().split(",")) {
				...
				if (!Utils.filterNa(rs.getLinghubAttributes().getDescription()).isEmpty())
					mainDataset.addProperty(dcDescription, rs.getLinghubAttributes().getDescription());
				...
				if (!Utils.filterNa(rs.getLinghubAttributes().getEmailContact()).isEmpty()) {
					...
					contactPoint.addProperty(vcardHasEmail, rs.getLinghubAttributes().getEmailContact());
			...
			if (!found) {
				...
				if (!Utils.filterNa(rs.getLinghubAttributes().getRights()).isEmpty())
					linghubDistro.addProperty(dcRights, rs.getLinghubAttributes().getRights());
			...
			if (!relFilePath.isEmpty()) {
				...
				for (LanguageMatch lm : rs.getFileInfo().getSelectedLanguages()) {
					...
					switch (rs.getFileInfo().getFileFormat()) {
