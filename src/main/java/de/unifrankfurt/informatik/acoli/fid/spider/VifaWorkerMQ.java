...
public class VifaWorkerMQ implements Runnable, Worker  {
			...
			for (String folder : filesInDirMap.keySet()) {
				...
				for (String path : filesInDirMap.get(folder)) {
					...
					if(rdfFiles.contains(path)) {
						...
						ResourceInfo resourceInfo_ = makeFileResource(resourceInfo, path, FileFormat.RDF, false);
					    ...
					    */
						...
						send2OutQueue(resourceInfo_, ResourceProcessState.INPROGESS);
				...
				for (String path : filesInDirMap.get(folder)) {
					...
					if(conllFilesByExtension.contains(path)) {
						...
						if (f.isFile()) { // always true
							...
							// Create new resourceInfo object for file
							ResourceInfo resourceInfo_ = makeFileResource(resourceInfo, path, FileFormat.CONLL, false);
							foundCONLL = true;
			...
			for (String folder : filesInDirMap.keySet()) {
				...
				for (String path : filesInDirMap.get(folder)) {
					...
					if(xmlFiles.contains(path)) {
						...
						// Create new resourceInfo object for file
						ResourceInfo resourceInfo_ = makeFileResource(resourceInfo, path, FileFormat.XML, false);
	...
	private void setFileTypeNotSupported(ResourceInfo resourceInfo) {
		...
		Vertex fileVertex = downloadManager.getResourceManager().addFile(resourceInfo, FileFormat.UNKNOWN);
	...
	private ResourceInfo makeFileResource(ResourceInfo resourceInfo, String filePath, FileFormat fileFormat, boolean single) {
		...
		resourceInfo_.setLinghubAttributes(resourceInfo.getLinghubAttributes());
